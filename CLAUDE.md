...
**2º BOT de exemplo:** `bots/bot_ip/` (v0.3) — recebe um texto, extrai os **IPs de WAN** (regex + `ipaddress.is_global`, descarta privados/reservados/CGNAT) e, num loop por IP, consulta **três fontes**: **Shodan** (exposição: portas/serviços/vulns), **AbuseIPDB** (reputação: abuse score/reports) e **URLhaus** da abuse.ch (distribuição de malware: `POST /v1/host/` → URLs maliciosas/famílias/tags servidas pelo IP — `task03b_urlhaus.py`, node `02_03_urlhaus`, acumula `$prev`). Consolida num **relatório PDF** (1 IP/página, ordenado por abuse→malware→exposição) e **publica num endpoint de upload** (PUT/POST). **API keys e destino de upload vivem no ENV DO DRONE** (`SHODAN_API_KEY`/`ABUSEIPDB_API_KEY`/`URLHAUS_API_KEY`/`MYASS_UPLOAD_*` em `/opt/myass/drone.env` 600 via `EnvironmentFile=`) — nunca no pedido. A task de **upload só roda se houver URL E token** (senão pula sem falhar; o PDF sai inline). Mesmas libs do `bot_cve` (`io`/`http`/`minipdf`) + `lib/net.py`; `lib/http.py` ganhou `post_form_json` (form-urlencoded → JSON, padrão do URLhaus); `build.py` gera `manifest.json`/`workflow.json`. **Comprovadamente rodado fim-a-fim no quadrante real** e publicado em produção (autorizado também na chave web). Espelha a invariante de pulverização e o contrato de scripts.

...
**Lastro físico do Locutus (DEFINIDO — decisão do dono): hosting banal, acesso banal HTTPS.** Qualquer hospedagem comum que sirva HTTPS com PUT/GET de blobs resolve — nenhuma infra especial, porque a segurança não depende do hosting: o conteúdo é E2E (ChaCha20, ver *Borda do cliente*) e o servidor é cego por construção. O hosting é **descartável/substituível** (blobs efêmeros com TTL; trocar de provedor não custa nada). O provedor vê metadado de acesso (IPs de clientes, padrões), nunca conteúdo — aceito. Polling do núcleo: Tor preferencial, surface permitida (ver *Canais seguros → Transporte*).
